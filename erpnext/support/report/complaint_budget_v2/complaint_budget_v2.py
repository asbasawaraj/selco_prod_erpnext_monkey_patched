...
def execute(filters=None):
	...
	for d in issue_list1:
		if d.special_budget_approved_by_csd>0:
			...
			var1=0.35*d.service_amount
		else:
			var1=0
		row = [d.name,d.customer_full_name,d.service_person,d.service_record_number,d.service_amount,var1,d.csd_remarks]
		data.append(row)
...
def get_sr_details(filters):
	...
	return frappe.db.sql("""SELECT A.name,A.date,A.customer_full_name,A.ics_date,A.remarks,B.service_record_number,B.service_person,B.service_record_date,B.service_amount,B.within_warranty,B.special_budget_approved_by_csd,B.budget_thirty_five,B.csd_remarks FROM `tabIssue` AS A INNER JOIN `tabService Record Details Issue` AS B ON A.name=B.parent WHERE service_branch_email_id = %s AND B.service_record_date BETWEEN %s AND %s""", (selco_branch,msd,med),as_dict=1)
