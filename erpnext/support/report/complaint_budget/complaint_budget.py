...
def get_details_branch(filters):
	...
	selco_service_person = filters.get("selco_service_person")
	if selco_service_person:
		return frappe.db.sql("""select name, date, customer_full_name, ics_date, complaint_handled_by,special_budget,service_charges_collected,service_record_number,remarks from `tabIssue` where workflow_state="Complaint Closed By Branch" AND branch = %s AND complaint_closed_date BETWEEN %s AND %s AND complaint_handled_by LIKE %s""", (selco_branch,msd,med,selco_service_person),as_dict=1)
	else:
		return frappe.db.sql("""select name, date, customer_full_name, ics_date, complaint_handled_by,special_budget,service_charges_collected,service_record_number,remarks from `tabIssue` where workflow_state="Complaint Closed By Branch" AND branch = %s AND complaint_closed_date BETWEEN %s AND %s""", (selco_branch,msd,med),as_dict=1)
	...
	#return frappe.db.sql("""select name, customer_full_name from `tabIssue` where workflow_state = "Complaint Closed By CSD" %s"""
