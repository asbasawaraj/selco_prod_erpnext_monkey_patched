...
#Start entering of code by Poorvi on 30-09-20016
        var4 = frappe.db.get_value("Customer", {"customer_contact_number": (self.customer_contact_number)})
        var5 = unicode(var4) or u''
        var6 = frappe.db.get_value("Customer", {"customer_contact_number": (self.customer_contact_number)}, "customer_name")
        if var5 != "None" and self.name != var5:
            frappe.throw("Customer with contact no " + self.customer_contact_number + " already exists \n Customer ID : " + var5 + "\n Customer Name : " + var6)
#End of entering Code
