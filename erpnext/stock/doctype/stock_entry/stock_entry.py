...
class StockEntry(StockController):
    ...
    def validate(self):
        ...
        if self.purpose=="Material Transfer":
            selco_cost_center = frappe.db.get_value("Warehouse",self.to_warehouse,"cost_center")
            for d in self.get('items'):
                d.cost_center = selco_cost_center
        if self.purpose == "Material Issue":
            selco_cost_center = frappe.db.get_value("Warehouse",self.from_warehouse,"cost_center")
            for d in self.get('items'):
...
@frappe.whitelist()
def get_items_from_ref_doc(selco_doc_num,selco_branch):
